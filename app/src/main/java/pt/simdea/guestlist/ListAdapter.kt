...
class ListAdapter(private val modelsArrayList: MutableList<Item>) : RecyclerView.Adapter<ListAdapter.RowViewHolder>() {
    ...
    override fun getItemCount(): Int = modelsArrayList.size

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ListAdapter.RowViewHolder =  RowViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.row, parent, false))

    override fun onBindViewHolder(holder: ListAdapter.RowViewHolder, position: Int) {
    ...
    fun getItem(position: Int): Item = modelsArrayList[position]
     ...
     */
    private fun validateItemPosition(index: Int): Boolean {
        return index in 1..(itemCount - 1)
    }
     ...
     */
    private fun validateItemPosition(item: Item): Int {
        return if (modelsArrayList.contains(item)) modelsArrayList.indexOf(item) else itemCount
    }
     ...
     */
    private fun validateLastItemPosition(): Int {
        return if (itemCount - 1 < 0) 0 else itemCount - 1
    }
